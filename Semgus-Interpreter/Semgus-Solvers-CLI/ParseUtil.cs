...
namespace Semgus.CommandLineInterface {
    public static class ParseUtil {
        public class TypicalItems {
            ...
            public TypicalItems(SmtContext smt, SemgusContext sem, SemgusSynthFun synthFun, InterpretationLibrary library, InterpretationGrammar grammar, InductiveConstraint constraint) {
                Smt = smt;
                Sem = sem;
                SynthFun = synthFun;
                Library = library;
            ...
            public static TypicalItems Acquire(string filePath, ILogger logger = null) {
                var (smt, sem) = ParseUtil.ParseFile(filePath, logger);
                ...
                return new(smt, sem, sf, lib, grammar, spec);
            }
        }

        public static (SmtContext smt, SemgusContext sem) ParseFile(string filePath,ILogger logger = null) {
            if(TryParseFile(filePath,logger,out var smt, out var sem)) {
                return (smt, sem);
            } else {
        ...
        public static bool TryParseFile(string filePath, ILogger logger, out SmtContext smt, out SemgusContext sem) {
        ...
        private class OneShotHandler : ISemgusProblemHandler {
            ...
            public void OnSetInfo(SmtContext ctx, SmtAttribute attr) {
                if (HitCheckSynth) throw new NotSupportedException("set-info after check-synth");
